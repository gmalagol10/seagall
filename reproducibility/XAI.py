...
jc=pd.DataFrame(index=fts.columns, columns=fts.columns)
for column in jc.columns:
	for col in jc.columns:
		if len(fts[column].dropna())==0 or len(fts[col].dropna())==0:
			print(f"Problem with either {column} or {col}")
		else:	
			jc.at[column, col]=len(ut.intersection([fts[column].dropna(), fts[col].dropna()]))/len(ut.flat_list([fts[column].dropna(), fts[col].dropna()]))
jc.to_csv(f"{de_path}_DETop{str(n_feat)}FeaturesJaccard.tsv.gz", sep="\t", compression="gzip")	   		
