...
def split_train_val_GR_AE(X, E, y, train_size=0.85, test_size=0.15, train_batch_size=128, val_batch_size=64):
	...
	if len(y)==0:
		print("No label to sample the dataset")
		y=np.ones(shape=(M.shape[0],))
		y=np.array(y).astype(int)
		X=scipy.sparse.csr_matrix(X, dtype="float32").todense()
		y=np.array(y).astype(int)

		X_train, X_val, E_train, E_val, y_train, y_val = sklearn.model_selection.train_test_split(X, E, y, train_size=train_size, test_size=test_size, random_state=42, stratify=y)
		y_train=y_train.astype(int)	
		y_val=y_val.astype(int)	
		
		trainDataset = torch.utils.data.TensorDataset(torch.tensor(X_train, dtype=torch.float32), torch.tensor(E_train, dtype=torch.float32), torch.LongTensor(y_train))
		...
		valDataset = torch.utils.data.TensorDataset(torch.tensor(X_val, dtype=torch.float32), torch.tensor(E_val, dtype=torch.float32), torch.LongTensor(y_val))
...
class LinearAutoencoder(nn.Module):
	def __init__(self, input_dim, latent_dim, hidden_dim_2=800, hidden_dim_1=400, hidden_dim=200, dp=0.5, act_fn=torch.nn.LeakyReLU, loss_fn=torch.nn.MSELoss()):
		...
		self.encoder = torch.nn.Sequential(torch.nn.Dropout(dp), torch.nn.Linear(self.input_dim, self.hidden_dim_2), self.act_fn(), 
										   torch.nn.Dropout(dp), torch.nn.Linear(self.hidden_dim_2, self.hidden_dim_1), self.act_fn(), 
										   torch.nn.Dropout(dp), torch.nn.Linear(self.hidden_dim_1, self.hidden_dim), self.act_fn(),
		...
		self.decoder = torch.nn.Sequential(torch.nn.Dropout(dp), torch.nn.Linear(self.latent_dim, self.hidden_dim), self.act_fn(), 
										   torch.nn.Dropout(dp), torch.nn.Linear(self.hidden_dim, self.hidden_dim_1), self.act_fn(), 
										   torch.nn.Dropout(dp), torch.nn.Linear(self.hidden_dim_1, self.hidden_dim_2), self.act_fn(), 
										   torch.nn.Dropout(dp), torch.nn.Linear(self.hidden_dim_2, self.input_dim))
...
class GR_AE(AutoencoderModel):
	def __init__(self, input_dim, lam=100, latent_dim=32):
		super().__init__()
		...
		self.autoencoder = LinearAutoencoder(latent_dim=self.latent_dim, input_dim=self.input_dim)
	...
	def forward(self, x, emb):
		...
		geom_loss = self.loss_fn(z, emb)

		loss = rec_loss.type(torch.float32) + self.lam * geom_loss.type(torch.float32)
...
def TrainGRAEs(adata, y=[], params=None, model_name="Pippo", layer="X"):
	if layer == "X":
		M=adata.X.copy()
	else:
	...
	if params is None:
		params={}
		params["dp"]=0.5
		params["input_dim"]=int(M.shape[1])
	...
	ae_kwargs={"lam" : params["lam"], "input_dim" : params["input_dim"], "latent_dim" : params["latent_dim"]}

	if "PHATE" in adata.obsm.keys():
		print(f"Found PHATE representation to preserve in latent space", flush=True)
		Y_phate=adata.obsm["PHATE"].copy()
	else:
		phate_operator = phate.PHATE(n_jobs=-2, n_components=params["latent_dim"])
		Y_phate = phate_operator.fit_transform(M)

	if len(y) == 0:
		y=np.ones(shape=(M.shape[0],))
	y=np.array(y).astype(int)

	train_dataloader, val_dataloader = split_train_val_GR_AE(M, Y_phate, y)

	model = GR_AE(ae_kwargs)
	optimizer = torch.optim.Adam(model.parameters(), lr=params["lr"], weight_decay=params["weight_decay"])

	print("AE", model, flush=True)	
	best_val_loss = 10e10
	best_epoch = -1
	torch.save(model.state_dict(), f"{model_name}.pth")
	learning_data={}
	learning_data["Train_Loss"]=[]
	learning_data["Val_Loss"]=[]

	for epoch in range(0, params["epochs"]):
		val_losses=[]
		train_losses=[]
		for X, emb, Y in train_dataloader:
			train_loss = model(X, emb)
			optimizer.zero_grad()
			train_loss.backward()
			optimizer.step()
			train_losses.append(train_loss.item())
		learning_data["Train_Loss"].append(np.float64(np.mean(train_losses)))

		for X, emb, Y in val_dataloader:
			val_losses.append(model(X, emb).item())
		val_loss=np.mean(val_losses)
		learning_data["Val_Loss"].append(np.float64(val_loss))
		...
		if np.around(val_loss/best_val_loss, decimals=2) < 0.95:
			...
			model.eval()
			val_losses=[model(X, emb).item() for X, emb, Y in val_dataloader]			
			...
			with open(f"{model_name}_LearningData.json", 'w') as f:
				json.dump(learning_data, f)
			return model.encode(dataset).detach().numpy(), scipy.sparse.csr_matrix(model.decode(model.encode(dataset)).detach().numpy(),  dtype="float32")
